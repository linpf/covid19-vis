...
def prov_testing_bar_chart(province):
    ...
    chart.title = f"daily testing in {province}"
    chart.x_labels = sorted_report_days 
    chart.render_to_file('prov_testing_bar_chart.svg')
...
def prov_cumulative_testing_line_chart(province):
    ...
    sorted_report_days = sorted(list(report_days),key=day_month_year)
    chart = pygal.Line(height=360,show_x_labels=False,show_legend=False)
    province_series_data = []
    for day in sorted_report_days:
        if (day,province) in data_x_y:
            province_series_data.append(data_x_y[(day,province)])
            ...
            province_series_data.append(None)
    chart.add(province, province_series_data)
    chart.title = f"cumulative testing in {province}"
    chart.x_labels = sorted_report_days 
    chart.render_to_file('prov_cumulative_testing_line_chart.svg')

def provs_testing_stackedbar_chart():
    with open("../data/Covid19Canada/timeseries_prov/testing_timeseries_prov.csv", 'r') as file:
        data_x_y = {} 
        provinces_list = {} 
        csv_file = csv.DictReader(file)
...
def provs_cumulative_testing_line_chart():
    with open("../data/Covid19Canada/timeseries_prov/testing_timeseries_prov.csv", 'r') as file:
        data_x_y = {} 
        provinces_list = {} 
        csv_file = csv.DictReader(file)
    ...
    chart.x_labels = sorted_report_days 
    chart.render_to_file('provs_cumulative_testing_line_chart.svg')

if __name__ == "__main__":
    provs_cumulative_testing_line_chart()
    provs_testing_stackedbar_chart()
    prov_cumulative_testing_line_chart("BC")
    prov_testing_bar_chart("BC")
