...
def canada_mortality_line_chart():
    with open("../data/Covid19Canada/timeseries_canada/mortality_timeseries_canada.csv", 'r') as file:
        ...
        for row in csv_file:
            ...
            data_x_y[(row_data["date_death_report"], row_data["province"])] = int(
                row_data["deaths"])
            groups_list[row_data["province"]] = int(
    ...
    chart.title = "Canada cumulative deaths"
    chart.x_labels = sorted_report_days
    chart.render_to_file('canada_cumulative deaths_line_chart.svg')
    ...
    for n, group in enumerate(groups):
        for d in range(6, len(report_days)):
            ...
            for i in range(d - 6, d + 1):
                data_7days_total += data_x_y.get(
                    (sorted_report_days[i], group), 0)
