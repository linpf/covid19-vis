...
def provs_mortality_cumulative_hbar_chart(request):
    ...
    chart = pygal.HorizontalStackedBar(height=320, legend_at_bottom=True)
...
def provs_testing_line_chart():
    ...
    sorted_report_days = sorted(list(report_days), key=day_month_year)
    chart = pygal.Line(height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01,
...
def provs_cases_weekly_bar_chart(request):
    ...
    sorted_report_weeks = sorted(report_weeks)
    chart = pygal.StackedBar(height=320, show_x_labels=True, show_legend=True, legend_at_bottom=True)
    #chart = pygal.Bar(height=320,legend_at_bottom=True)
    for province in sorted_provinces:
        timeseries_data = []
        for week in sorted_report_weeks:
...
def prov_hrs_cases_weekly_bar_chart(request, province):
    ...
    sorted_report_weeks = sorted(report_weeks)
    chart = pygal.StackedBar(height=320, show_x_labels=True, show_legend=False)
...
def provs_cases_line_chart(request):
    ...
    chart = pygal.StackedBar(height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01,
...
def prov_cases_timeseries_stacked_bar_chart():
    ...
    chart = pygal.StackedBar(
        height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01, legend_at_bottom=False)
    for province in sorted_provinces:
        province_cases_per_day = []
        for day in sorted_report_days:
            if (day, province) in data_x_y:
...
def provs_latest_cases_and_mortality_stackbar_chart(request):
    ...
    chart = pygal.HorizontalStackedBar(height=320, 
        legend_at_bottom=True)
...
def hrs_latest_cases_and_mortality_stackbar_chart(request):
    ...
    chart = pygal.HorizontalStackedBar(height=320, show_legend=False,legend_at_bottom=False)
...
def provs_cumulative_cases_stackbar_chart2():
    ...
    sorted_report_provinces = sorted(
        report_provinces, key=lambda k: -data_x_y[(k, "cumulative_cases")])
    chart = pygal.StackedBar(height=320, show_legend=True, 
...
def provs_cumulative_cases_stackbar_chart3(request):
    ...
    sorted_report_provinces = sorted(
        report_provinces, key=lambda k: -data_x_y[(k, "cumulative_cases")])
    chart = pygal.StackedBar(height=320, legend_at_bottom=True)
    for province in sorted_report_provinces:
        cumulative_data_list = []
        for group in sorted_groups:
            if (province, group) in data_x_y:
...
def provs_testing_compared_line_chart():
    ...
    sorted_provinces = sorted(provinces_list.keys(),
                              key=lambda k: -provinces_list[k])
    sorted_report_days = sorted(list(days_report), key=day_month_year)
    chart = pygal.Line(height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01, legend_at_bottom=True)
    for province in sorted_provinces:
        province_cases_per_day = []
        for day in sorted_report_days:
            if (day, province) in data_x_y:
...
def prov_hrs_mortality_bar_chart(province):
    ...
    sorted_report_days = sorted(list(report_days), key=day_month_year)
    chart = pygal.Bar(
        height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01, show_legend=True, 
...
def prov_hrs_cases_bar_chart(province):
    ...
    sorted_hrs = sorted(hrs_list.keys(), key=lambda k: -hrs_cumulative[k])
    sorted_report_days = sorted(list(report_days), key=day_month_year)
    chart = pygal.Bar(height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01, show_legend=True,
...
def prov_hrs_cumulative_cases_and_mortality_stacked_bar_chart(request, province):
    ...
    chart = pygal.HorizontalStackedBar(height=320, legend_at_bottom=True, show_x_labels=True)
    for hr in sorted_report_hrs:
        data_list = []
        for group in sorted_groups:
...
def prov_cases_bar_chart(province):
    ...
    sorted_groups = sorted(groups_list.keys(), key=lambda k: -groups_list[k])
    sorted_report_days = sorted(list(report_days), key=day_month_year)
    chart = pygal.Bar(height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01, show_legend=False)
    for group in sorted_groups:
        data_list = []
        for day in sorted_report_days:
            if (day, group) in data_x_y:
...
def prov_testing_bar_chart(province):
    ...
    sorted_report_days = sorted(list(report_days), key=day_month_year)
    chart = pygal.Bar(height=360, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01, show_legend=False)
...
def prov_cases_and_testing_bar_chart(province):
    ...
    sorted_report_days = sorted(list(report_days), key=day_month_year)
    chart = pygal.Bar(height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01, legend_at_bottom=True)
    for group in ["cases", "testing"]:
        data_list = []
        for day in sorted_report_days:
            if (day, group) in data_x_y:
...
def prov_cumulative_testing_line_chart(province):
    ...
    sorted_report_days = sorted(list(report_days), key=day_month_year)
    chart = pygal.Line(height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01, show_legend=False)
    province_series_data = []
    for day in sorted_report_days:
        if (day, province) in data_x_y:
            province_series_data.append(data_x_y[(day, province)])
...
def prov_hrs_cumulative_mortality_line_chart(province):
    ...
    sorted_report_days = sorted(list(report_days), key=day_month_year)
    chart = pygal.Line(height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01, dots_size=2,
        legend_at_bottom=True,style=DarkSolarizedStyle)
    for hr in sorted_hrs:
        series_data_list = []
        for day in sorted_report_days:
...
def hr_cases_and_mortality_bar_chart(province, health_region):
    ...
    sorted_report_days = sorted(list(report_days), key=day_month_year)
    chart = pygal.Bar(height=320, show_x_labels=True, show_minor_x_labels=False, x_label_rotation=0.01, 
        legend_at_bottom=True, show_legend=True)
    for group in [ "deaths", "cases"]:
        data_list = []
        for day in sorted_report_days:
...
def hr_cumulative_hbar_chart(request, province, health_region):
    ...
    chart = pygal.HorizontalBar(height=320, show_x_labels=True, legend_at_bottom=True)
    for group in [ "cumulative_deaths" , "cumulative_cases" ]:
        cumulative_data_list = []
...
def provs_hrs_cases_pie_chart(request):
    ...
    chart = pygal.Pie(height=320,legend_box_size=18)
...
def provs_hrs_cumulative_cases_pie_chart(request):
    ...
    chart = pygal.Pie(height=320, show_legend=True, truncate_legend=-1)
